...
- fill_text 直接書寫填空（最常見，例如寫國字、注音、改錯字、數學數值/單位）：
  - options 留空陣列 []。
  - answer 放「所有可接受的正確寫法」字串陣列（系統會自動忽略空白與全形半形差異）。數值放 ["19"]、國字放 ["殼"]；若有多種寫法都對就全列。
  - stem 用 ___ 標出要填的空格。
- fill_blank 同音字選項：只有考卷明確要求「從同音不同字的選項中選」時才用；否則一律用 fill_text。
